...
urlpatterns = [
    ...
    url(r'^(?P<owner_slug>[\w-]+)/(?P<company_slug>[\w-]+)/stores-list/$', views.stores_list_view, name="stores_list"),
    ...
    url(r'^(?P<company_slug>[\w-]+)/(?P<store_slug>[\w-]+)/update-store$', views.StoreUpdateView.as_view(), name="update_store"),
    url(r'^(?P<company_slug>[\w-]+)/(?P<store_slug>[\w-]+)/delete-store$', views.store_delete_view, name="delete_store"),
]
